...
public class ExportssServlet extends HttpServlet {
	...
	private void exportEXcel(HttpServletRequest req, HttpServletResponse resp) throws Exception {
		...
		if (exportList.get(0).getRole_Level().equals("讲师")) {
			...
			String[] headName2 = { "大区", "校区", "教师姓名", "角色", "专业", "班级", "老师出勤", "关心程度", "巡堂", "找学员沟通", "缺勤关注", "班级纪律",
					"受理投诉", "组织活动", "资料的及时收发", "整体工作评分", "总分", "平均分", "学员建议" };
		...
		//
		// String Column[] = { "large_Area", "sch_Name", "tea_Name",
		// "role_Level", "cus_Name", "stu_Class",
		// "tea_Attendance", "cls_Explain", "cls_Quesions", "ques_Answer",
		// "cls_Coach", "cls_Discipline",
		// "cls_Skill", "cls_Progress", "exam_Explain", "class_Homework",
		// "total_Score", "average", "stu_Advice" };// 列id
		// ExportUtils.outputHeaders(headName, sheets, headerStyle);// 生成表头
		// ExportUtils.outputColumn(Column, exportList, sheets, 1,
		// cell_Style);// 生成列表数据
		//
	...
	private void expressTeacher(HttpServletRequest req, HttpServletResponse resp, String[] headName,
		...
		out.flush();
		out.close();
		req.getSession().removeAttribute("selectInfo");
	}
